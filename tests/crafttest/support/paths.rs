...
fn global_root() -> PathBuf {
    ...
    // If `craft test` is run manually then our path looks like
    // `target/debug/foo`, in which case our `path` is already pointing at
    // `target`. If, however, `craft test --target $target` is used then the
    ...
    path.join(CRAFT_INTEGRATION_TEST_DIR)
}
