...
use std::{fmt, env, fs, os, str, usize};

use rustc_serialize::json::Json;
use url::Url;
use hamcrest::core as hamcrest;
use craft::util::ProcessBuilder;
use craft::util::ProcessError;

use support::paths::CraftPathExt;
...
impl ProjectBuilder {
    ...
    pub fn craft(&self, cmd: &str) -> ProcessBuilder {
        let mut p = self.process(&craft_dir().join("craft"));
        p.arg(cmd);
        return p;
    }

    pub fn craft_process(&self, cmd: &str) -> ProcessBuilder {
        self.build();
        self.craft(cmd)
    }
