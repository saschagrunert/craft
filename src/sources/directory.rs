...
use core::{Package, PackageId, SourceId, Source, Dependency};
    ...
    fn update(&mut self) -> CraftResult<()> {
        ...
        for entry in entries {
            ...
            let cksum = try!(paths::read(&path.join(cksum_file)).chain_error(|| {
                human(format!("failed to load checksum `.craft-checksum.json` \
                               of {} v{}",
                              pkg.package_id().name(),
            ...
            let cksum: Checksum = try!(json::decode(&cksum).chain_error(|| {
                human(format!("failed to decode `.craft-checksum.json` of \
                               {} v{}",
                              pkg.package_id().name(),
                              pkg.package_id().version()))
            }));
