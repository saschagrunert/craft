...
pub fn install(root: Option<&str>,
    ...
    let overidden_target_dir = if source_id.is_path() {
        None
    } else if let Ok(td) = TempDir::new("craft-install") {
    ...
    let result = {
        let mut try_install = || -> CraftResult<()> {
    ...
    let pkgs = list.v1
        .iter()
        .filter_map(|(p, set)| if set.is_empty() {
            Some(p.clone())
        } else {
            None
        })
...
fn select_pkg<'a, T>(mut source: T,
                     ...
                     name: Option<&str>,
                     vers: Option<&str>,
                     list_all: &mut FnMut(&mut T) -> CraftResult<Vec<Package>>)
        ...
        Some(name) => {
            ...
            match deps.iter().map(|p| p.package_id()).max() {
                ...
                None => {
                    ...
                    Err(human(format!("could not find `{}` in `{}`{}", name, source_id, vers_info)))
                }
            }
        }
        None => {
            ...
            fn multi_err(kind: &str, mut pkgs: Vec<&Package>) -> String {
                ...
                format!("multiple packages with {} found: {}",
                        kind,
                        pkgs.iter()
                            .map(|p| p.name())
                            .collect::<Vec<_>>()
...
fn one<I, F>(mut i: I, f: F) -> CraftResult<Option<I::Item>>
    where I: Iterator,
...
fn check_overwrites(dst: &Path,
    ...
    if let CompileFilter::Everything = *filter {
        ...
        // get checked during craft_compile, we only care about the "build
