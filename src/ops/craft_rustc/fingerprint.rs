...
pub fn prepare_target<'a, 'cfg>(cx: &mut Context<'a, 'cfg>, unit: &Unit<'a>) -> CraftResult<Preparation> {
    ...
    if compare.is_err() {
        ...
        let source = sources.get(source_id).chain_error(|| internal("missing package source"))?;
    ...
    fn update_local(&self) -> CraftResult<()> {
        match self.local {
            LocalFingerprint::MtimeBased(ref slot, ref path) => {
                let meta = fs::metadata(path).chain_error(|| internal(format!("failed to stat `{}`", path.display())))?;
...
impl Encodable for Fingerprint {
    fn encode<E: Encoder>(&self, e: &mut E) -> Result<(), E::Error> {
        e.emit_struct("Fingerprint", 6, |e| {
            ...
            e.emit_struct_field("features", 4, |e| self.features.encode(e))?;
...
fn compare_old_fingerprint(loc: &Path, new_fingerprint: &Fingerprint) -> CraftResult<()> {
    ...
    let old_fingerprint =
        json::decode(&old_fingerprint_json).chain_error(|| internal(format!("failed to deserialize json")))?;
