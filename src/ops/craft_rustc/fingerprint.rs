...
///
/// When dealing with fingerprints, craft gets to choose what granularity
...
pub fn prepare_target<'a, 'cfg>(cx: &mut Context<'a, 'cfg>, unit: &Unit<'a>) -> CraftResult<Preparation> {
    ...
    if compare.is_err() {
        ...
        let source = try!(sources.get(source_id).chain_error(|| internal("missing package source")));
...
impl Fingerprint {
    fn update_local(&self) -> CraftResult<()> {
        match self.local {
            LocalFingerprint::MtimeBased(ref slot, ref path) => {
                let meta = try!(fs::metadata(path)
                    .chain_error(|| internal(format!("failed to stat `{}`", path.display()))));
...
impl Encodable for Fingerprint {
    fn encode<E: Encoder>(&self, e: &mut E) -> Result<(), E::Error> {
        e.emit_struct("Fingerprint", 6, |e| {
            ...
            try!(e.emit_struct_field("features", 4, |e| self.features.encode(e)));
...
impl Decodable for MtimeSlot {
    fn decode<D: Decoder>(e: &mut D) -> Result<MtimeSlot, D::Error> {
        ...
        Ok(MtimeSlot(Mutex::new(kind.map(|(s, n)| FileTime::from_seconds_since_1970(s, n)))))
    }
}

/// Calculates the fingerprint for a package/target pair.
///
/// This fingerprint is used by Craft to learn about when information such as:
///
...
fn calculate<'a, 'cfg>(cx: &mut Context<'a, 'cfg>, unit: &Unit<'a>) -> CraftResult<Arc<Fingerprint>> {
    ...
    let deps = try!(deps.iter()
        .filter(|u| !u.target.is_custom_build() && !u.target.is_bin())
        .map(|unit| calculate(cx, unit).map(|fingerprint| (unit.pkg.package_id().to_string(), fingerprint)))
...
pub fn prepare_build_cmd<'a, 'cfg>(cx: &mut Context<'a, 'cfg>, unit: &Unit<'a>) -> CraftResult<Preparation> {
    ...
    let write_fingerprint = Work::new(move |_| {
        if let Some(output_path) = output_path {
            ...
            if !outputs[&key].rerun_if_changed.is_empty() {
                ...
                fingerprint.local = LocalFingerprint::MtimeBased(slot, output_path);
...
fn write_fingerprint(loc: &Path, fingerprint: &Fingerprint) -> CraftResult<()> {
...
pub fn prepare_init(cx: &mut Context, unit: &Unit) -> CraftResult<()> {
...
fn compare_old_fingerprint(loc: &Path, new_fingerprint: &Fingerprint) -> CraftResult<()> {
    ...
    let old_fingerprint = try!(json::decode(&old_fingerprint_json)
        .chain_error(|| internal(format!("failed to deserialize json"))));
    new_fingerprint.compare(&old_fingerprint)
}

fn log_compare(unit: &Unit, compare: &CraftResult<()>) {
    let mut e = match *compare {
        Ok(..) => return,
        Err(ref e) => &**e,
    };
    info!("fingerprint error for {}: {}", unit.pkg, e);
    while let Some(cause) = e.craft_cause() {
        info!("  cause: {}", cause);
        e = cause;
    }
    let mut e = e.cause();
...
fn dep_info_mtime_if_fresh(dep_info: &Path) -> CraftResult<Option<FileTime>> {
    macro_rules! fs_try {
    ...
    loop {
        ...
        while file.ends_with("\\") {
            file.pop();
            file.push(' ');
            file.push_str(try!(deps.next()
                .chain_error(|| internal(format!("malformed dep-info format, trailing \\")))));
        }
...
fn pkg_fingerprint(cx: &Context, pkg: &Package) -> CraftResult<String> {
          ...
          I::Item: AsRef<Path>
{
    ...
    if any_stale { None } else { Some(mtime) }
}
...
// next time.
pub fn append_current_dir(path: &Path, cwd: &Path) -> CraftResult<()> {
