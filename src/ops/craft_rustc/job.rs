...
use util::{CraftResult, Fresh, Dirty, Freshness};
...
pub struct Work {
    inner: Box<for<'a, 'b> FnBox<&'a JobState<'b>, CraftResult<()>> + Send>,
}
...
impl Work {
    pub fn new<F>(f: F) -> Work
        where F: FnOnce(&JobState) -> CraftResult<()> + Send + 'static
    {
    ...
    pub fn call(self, tx: &JobState) -> CraftResult<()> {
...
impl Job {
    ...
    /// computation.
    pub fn run(self, fresh: Freshness, state: &JobState) -> CraftResult<()> {
        match fresh {
