...
impl<'cfg> Compilation<'cfg> {
    ...
    /// See `process`.
    pub fn host_process<T: AsRef<OsStr>>(&self, cmd: T, pkg: &Package) -> CraftResult<ProcessBuilder> {
        self.fill_env(process(cmd), pkg, true)
    }

    /// See `process`.
    pub fn target_process<T: AsRef<OsStr>>(&self, cmd: T, pkg: &Package) -> CraftResult<ProcessBuilder> {
    ...
    fn fill_env(&self, mut cmd: ProcessBuilder, pkg: &Package, is_host: bool) -> CraftResult<ProcessBuilder> {
        ...
        cmd.env("CRAFT_MANIFEST_DIR", pkg.root())
            .env("CRAFT_PKG_VERSION_MAJOR", &pkg.version().major.to_string())
            .env("CRAFT_PKG_VERSION_MINOR", &pkg.version().minor.to_string())
            .env("CRAFT_PKG_VERSION_PATCH", &pkg.version().patch.to_string())
            .env("CRAFT_PKG_VERSION_PRE",
                 &pre_version_component(pkg.version()))
            .env("CRAFT_PKG_VERSION", &pkg.version().to_string())
            .env("CRAFT_PKG_NAME", &pkg.name())
            .env("CRAFT_PKG_DESCRIPTION",
                 metadata.description.as_ref().unwrap_or(&String::new()))
            .env("CRAFT_PKG_HOMEPAGE",
                 metadata.homepage.as_ref().unwrap_or(&String::new()))
            .env("CRAFT_PKG_AUTHORS", &pkg.authors().join(":"))
            .cwd(pkg.root());
        Ok(cmd)
    }
}
