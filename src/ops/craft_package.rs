...
fn check_metadata(pkg: &Package, config: &Config) -> CraftResult<()> {
...
fn check_not_dirty(p: &Package, src: &PathSource) -> CraftResult<()> {
        ...
        if let Some(workdir) = repo.workdir() {
            ...
            if let Ok(status) = repo.status_file(path) {
                if (status & git2::STATUS_IGNORED).is_empty() {
                    debug!("Craft.toml found in repo, checking if dirty");
...
fn tar(ws: &Workspace, src: &PathSource, dst: &File, filename: &str) -> CraftResult<()> {
    ...
    for file in try!(src.list_files(pkg)).iter() {
        ...
        //
        // Older crafts, however, had a bug where GNU archives were interpreted
        ...
        // older crafts.
        //
        // Hopefully in the future after enough crafts have been running around
...
fn check_filename(file: &Path) -> CraftResult<()> {
