//! Craft compile currently does the following steps:
//!
//! All configurations are already injected as environment variables via the
//! main craft command
//!
//! 1. Read the manifest
//! 2. Shell out to `craft-resolve` with a list of dependencies and sources as
//!    stdin
//!
...
pub fn compile<'a>(ws: &Workspace<'a>, options: &CompileOptions<'a>) -> CraftResult<ops::Compilation<'a>> {
...
pub fn resolve_dependencies<'a>(ws: &Workspace<'a>,
                                ...
                                spec: &'a [String])
                                -> CraftResult<(PackageSet<'a>, Resolve)> {
    ...
    if let Some(source) = source {
        registry.add_preloaded(try!(ws.current()).package_id().source_id(), source);
    }
    ...
    let specs = try!(spec.iter()
        .map(|p| PackageIdSpec::parse(p))
        .collect::<CraftResult<Vec<_>>>());

    let resolved_with_overrides =
        try!(ops::resolve_with_previous(&mut registry, ws, method, Some(&resolve), None, &specs));
...
pub fn compile_ws<'a>(ws: &Workspace<'a>,
    ...
    let (packages, resolve_with_overrides) = try!(resolve_dependencies(ws,
                                                                       source,
                                                                       features,
                                                                       all_features,
                                                                       no_default_features,
    ...
    match (*target_rustc_args, *target_rustdoc_args) {
        ...
        (Some(args), _) => {
            let targets = try!(generate_targets(to_builds[0], profiles, mode, filter, release));
            if targets.len() == 1 {
        ...
        (None, None) => {
            for &to_build in to_builds.iter() {
                let targets = try!(generate_targets(to_build, profiles, mode, filter, release));
...
impl<'a> CompileFilter<'a> {
    pub fn new(lib_only: bool,
               ...
               benches: &'a [String])
               -> CompileFilter<'a> {
        if lib_only || !bins.is_empty() || !tests.is_empty() || !examples.is_empty() || !benches.is_empty() {
            CompileFilter::Only {
                lib: lib_only,
                bins: bins,
                examples: examples,
...
fn generate_targets<'a>(pkg: &'a Package,
                        ...
                        release: bool)
                        -> CraftResult<Vec<(&'a Target, &'a Profile)>> {
        ...
        CompileFilter::Only { lib, bins, examples, tests, benches } => {
...

            {
                let mut find = |names: &[String], desc, kind, profile| {
                    for name in names {
                        let target = pkg.targets().iter().find(|t| t.name() == *name && *t.kind() == kind);
                        let t = match target {
                            Some(t) => t,
                            None => {
...
fn add_overrides<'a>(registry: &mut PackageRegistry<'a>, ws: &Workspace<'a>) -> CraftResult<()> {
    ...
    let paths = paths.val.iter().map(|&(ref s, ref p)| {
        ...
        // key was located, so we want to pop off the `.craft/config` component
        // to get the directory containing the `.craft` folder.
...
fn scrape_build_config(config: &Config, jobs: Option<u32>, target: Option<String>) -> CraftResult<ops::BuildConfig> {
...
fn scrape_target_config(config: &Config, triple: &str) -> CraftResult<ops::TargetConfig> {
        ...
        for (k, value) in try!(value.table(&lib_name)).0 {
            ...
            match &k[..] {
                "rustc-flags" => {
                    ...
                    let (paths, links) = try!(BuildOutput::parse_rustc_flags(&flags, &whence));
