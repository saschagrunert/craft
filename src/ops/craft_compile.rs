...
pub fn resolve_dependencies<'a>(ws: &Workspace<'a>,
    ...
    if let Some(source) = source {
        registry.add_preloaded(ws.current()?.package_id().source_id(), source);
    }
    ...
    let resolved_with_overrides = ops::resolve_with_previous(&mut registry, ws, method, Some(&resolve), None, &specs)?;
...
pub fn compile_ws<'a>(ws: &Workspace<'a>,
    ...
    let (packages, resolve_with_overrides) = resolve_dependencies(ws,
                                                                  source,
                                                                  features,
                                                                  all_features,
                                                                  no_default_features,
    ...
    match (*target_cc_args, *target_doc_args) {
        ...
        (None, None) => {
            for &to_build in to_builds.iter() {
                let targets = generate_targets(to_build, profiles, mode, filter, release)?;
