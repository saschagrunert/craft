...
impl Rustc {
    ...
    pub fn new(path: PathBuf) -> CraftResult<Rustc> {
        ...
        let verbose_version =
            String::from_utf8(output.stdout).map_err(|_| internal("rustc -v didn't return utf8 output"))?;

        let host = {
            let triple = verbose_version.lines()
                .find(|l| l.starts_with("host: "))
                .map(|l| &l[6..]);
            let triple = triple.chain_error(|| internal("rustc -v didn't have a line for `host:`"))?;
            triple.to_string()
        };
