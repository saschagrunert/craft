...
impl Config {
    ...
    fn get_env<V: FromStr>(&self, key: &str) -> CraftResult<Option<Value<V>>>
        where Box<CraftError>: From<V::Err>
    {
    ...
    pub fn expected<T>(&self, ty: &str, key: &str, val: CV) -> CraftResult<T> {
        val.expected(ty, key).map_err(|e| human(format!("invalid configuration for key `{}`\n{}", key, e)))
    }
    ...
    fn get_tool(&self, tool: &str) -> CraftResult<PathBuf> {
...
impl ConfigValue {
    fn from_toml(path: &Path, toml: toml::Value) -> CraftResult<ConfigValue> {
        match toml {
    ...
    fn merge(&mut self, from: ConfigValue) -> CraftResult<()> {
        match (self, from) {
            ...
            (expected, found) => {
                return Err(internal(format!("expected {}, but found {}", expected.desc(), found.desc())))
            }
        }
    ...
    pub fn i64(&self, key: &str) -> CraftResult<(i64, &Path)> {
        match *self {
    ...
    pub fn string(&self, key: &str) -> CraftResult<(&str, &Path)> {
        match *self {
    ...
    pub fn table(&self, key: &str) -> CraftResult<(&HashMap<String, ConfigValue>, &Path)> {
        match *self {
    ...
    pub fn list(&self, key: &str) -> CraftResult<&[(String, PathBuf)]> {
        match *self {
    ...
    pub fn boolean(&self, key: &str) -> CraftResult<(bool, &Path)> {
        match *self {
    ...
    fn expected<T>(&self, wanted: &str, key: &str) -> CraftResult<T> {
...
fn homedir(cwd: &Path) -> Option<PathBuf> {
    let craft_home = env::var_os("CRAFT_HOME").map(|home| cwd.join(home));
    if craft_home.is_some() {
        return craft_home;
    }

    // If `CRAFT_HOME` wasn't defined then we want to fall back to
    // `$HOME/.craft`. Note that currently, however, the implementation of
    ...
    // should help existing installs of Craft continue using the same craft home
    // directory.
    let home_dir_with_env = env::home_dir().map(|p| p.join(".craft"));
    let home_dir = env::var_os("HOME");
    env::remove_var("HOME");
    let home_dir_without_env = env::home_dir().map(|p| p.join(".craft"));
...
fn walk_tree<F>(pwd: &Path, mut walk: F) -> CraftResult<()>
    where F: FnMut(File, &Path) -> CraftResult<()>
{
    let mut current = pwd;
...
pub fn set_config(cfg: &Config, loc: Location, key: &str, value: ConfigValue) -> CraftResult<()> {
