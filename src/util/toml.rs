...
///
/// This structure is used to hold references to all project files that are relevant to craft.
#[derive(Clone)]
pub struct Layout {
    pub root: PathBuf,
    lib: Option<PathBuf>,
...
impl Layout {
    ...
    /// The `root_path` represents the directory that contains the `Craft.toml` file.
...
fn try_add_files(files: &mut Vec<PathBuf>, root: PathBuf) {
    match fs::read_dir(&root) {
        Ok(new) => {
            files.extend(new.filter_map(|dir| dir.map(|d| d.path()).ok())
                ...
                .filter(|f| {
                    ...
                    // that they are explicitly specified in Craft.toml if desired.
                    f.file_name()
                        .and_then(|s| s.to_str())
                        .map(|s| !s.starts_with('.'))
                        .unwrap_or(true)
        ...
        Err(_) => {
            // just don't add anything if the directory doesn't exist, etc.
        }
    }
}
...
pub fn to_manifest(contents: &str,
    ...
    let manifest = layout.root.join("Craft.toml");
    ...
    let manifest: TomlManifest = try!(Decodable::decode(&mut d).map_err(|e| human(e.to_string())));
...
pub fn parse(toml: &str, file: &Path, config: &Config) -> CraftResult<toml::Table> {
...
impl Decodable for TomlOptLevel {
    fn decode<D: Decoder>(d: &mut D) -> Result<TomlOptLevel, D::Error> {
        match d.read_u32() {
            ...
            Err(_) => {
                match d.read_str() {
                    Ok(ref s) if s == "s" || s == "z" => Ok(TomlOptLevel(s.to_string())),
                    Ok(_) | Err(_) => Err(d.error("expected an integer, a string \"z\" or a string \"s\"")),
                }
            }
        }
    }
...
impl TomlProject {
    pub fn to_package_id(&self, source_id: &SourceId) -> CraftResult<PackageId> {
        PackageId::new(&self.name, self.version.version.clone(), source_id)
    }
}
...
impl TomlManifest {
    fn to_real_manifest(&self,
                        ...
                        layout: &Layout,
                        config: &Config)
                        -> CraftResult<(Manifest, Vec<PathBuf>)> {
        let mut nested_paths = vec![];
        let mut warnings = vec![];

        let project = self.project.as_ref().or_else(|| self.package.as_ref());
        let project = try!(project.chain_error(|| human("no `package` or `project` section found.")));
                ...
                bins.iter()
                    .map(|t| {
                        if bin.is_some() && t.path.is_none() {
                            TomlTarget { path: bin.as_ref().map(|&p| PathValue::Path(p.clone())), ..t.clone() }
                        } else {
                            t.clone()
                        }
                    })
        ...
        for bin in bins.iter() {
            if blacklist.iter().find(|&x| *x == bin.name()) != None {
                bail!("the binary target name `{}` is forbidden", bin.name())
            }
        }
...

        {
            ...
            // Collect the deps
            try!(process_dependencies(&mut cx, self.dependencies.as_ref(), None));
            try!(process_dependencies(&mut cx,
                                      self.dev_dependencies.as_ref(),
                                      Some(Kind::Development)));
            try!(process_dependencies(&mut cx, self.build_dependencies.as_ref(), Some(Kind::Build)));

            for (name, platform) in self.target.iter().flat_map(|t| t) {
                cx.platform = Some(try!(name.parse()));
                try!(process_dependencies(&mut cx, platform.dependencies.as_ref(), None));
                try!(process_dependencies(&mut cx,
        ...
        let summary = try!(Summary::new(pkgid,
                                        deps,
                                        self.features
                                            .clone()
        ...
        let workspace_config = match (self.workspace.as_ref(), project.workspace.as_ref()) {
            (Some(config), None) => WorkspaceConfig::Root { members: config.members.clone() },
            (None, root) => WorkspaceConfig::Member { root: root.cloned() },
            (Some(..), Some(..)) => {
    ...
    fn to_virtual_manifest(&self,
                           ...
                           layout: &Layout,
                           config: &Config)
                           -> CraftResult<(VirtualManifest, Vec<PathBuf>)> {
        ...
        let workspace_config = match self.workspace {
            Some(ref config) => WorkspaceConfig::Root { members: config.members.clone() },
            None => {
...
impl TomlDependency {
    fn to_dependency(&self, name: &str, cx: &mut Context, kind: Option<Kind>) -> CraftResult<Dependency> {
        let details = match *self {
            TomlDependency::Simple(ref version) => {
                DetailedTomlDependency { version: Some(version.clone()), ..Default::default() }
            }
            TomlDependency::Detailed(ref details) => details.clone(),
        };

        if details.version.is_none() && details.path.is_none() && details.git.is_none() {
            let msg = format!("dependency ({}) specified without providing a local path, Git repository, or version \
        ...
        if details.git.is_none() {
            let git_only_keys = [(&details.branch, "branch"), (&details.tag, "tag"), (&details.rev, "rev")];
        ...
        let new_source_id = match (details.git.as_ref(), details.path.as_ref()) {
            (Some(git), maybe_path) => {
                if maybe_path.is_some() {
                    let msg = format!("dependency ({}) specification is ambiguous. Only one of `git` or `path` \
                ...
                if n_details > 1 {
                    let msg = format!("dependency ({}) specification is ambiguous. Only one of `branch`, `tag` \
                                           or `rev` is allowed. This will be considered an error in future versions",
                                      name);
                    cx.warnings.push(msg)
                }

                let reference = details.branch
                    .clone()
        ...
        let mut dep = match cx.pkgid {
            Some(id) => try!(DependencyInner::parse(name, version, &new_source_id, Some((id, cx.config)))),
...
impl TomlTarget {
    ...
    fn validate_library_name(&self) -> CraftResult<()> {
        match self.name {
            Some(ref name) => {
                if name.trim().is_empty() {
                    Err(human("library target names cannot be empty.".to_string()))
                } else if name.contains('-') {
                    Err(human(format!("library target names cannot contain hyphens: {}", name)))
                } else {
                    Ok(())
                }
            }
    ...
    fn validate_binary_name(&self) -> CraftResult<()> {
        match self.name {
            Some(ref name) => {
                if name.trim().is_empty() {
    ...
    fn validate_example_name(&self) -> CraftResult<()> {
        match self.name {
            Some(ref name) => {
                if name.trim().is_empty() {
    ...
    fn validate_test_name(&self) -> CraftResult<()> {
        match self.name {
            Some(ref name) => {
                if name.trim().is_empty() {
    ...
    fn validate_bench_name(&self) -> CraftResult<()> {
        match self.name {
            Some(ref name) => {
                if name.trim().is_empty() {
...
fn normalize(lib: &Option<TomlLibTarget>,
        ...
        let chest_types = match l.chest_type.clone() {
            ...
            None => {
                vec![if l.plugin == Some(true) {
                         LibKind::Dylib
                     } else if l.proc_macro == Some(true) {
                         LibKind::ProcMacro
                     } else {
                         LibKind::Lib
                     }]
    ...
    fn bin_targets(dst: &mut Vec<Target>, bins: &[TomlBinTarget], default: &mut FnMut(&TomlBinTarget) -> PathBuf) {
        for bin in bins.iter() {
            let path = bin.path.clone().unwrap_or_else(|| PathValue::Path(default(bin)));
            let mut target = Target::bin_target(&bin.name(), &path.to_path(), None);
    ...
    fn example_targets(dst: &mut Vec<Target>,
        ...
        for ex in examples.iter() {
            let path = ex.path.clone().unwrap_or_else(|| PathValue::Path(default(ex)));
    ...
    fn test_targets(dst: &mut Vec<Target>,
        ...
        for test in tests.iter() {
            let path = test.path.clone().unwrap_or_else(|| PathValue::Path(default(test)));

            // make sure this metadata is different from any same-named libs.
            let mut metadata = metadata.clone();
            metadata.mix(&format!("test-{}", test.name()));

            let mut target = Target::test_target(&test.name(), &path.to_path(), metadata);
    ...
    fn bench_targets(dst: &mut Vec<Target>,
        ...
        for bench in benches.iter() {
            let path = bench.path.clone().unwrap_or_else(|| PathValue::Path(default(bench)));

            // make sure this metadata is different from any same-named libs.
            let mut metadata = metadata.clone();
            metadata.mix(&format!("bench-{}", bench.name()));

            let mut target = Target::bench_target(&bench.name(), &path.to_path(), metadata);
    ...
    if let Some(ref lib) = *lib {
        ...
        bin_targets(&mut ret,
                    bins,
                    &mut |bin| Path::new("src").join(&format!("{}.rs", bin.name())));
    }
    ...
    example_targets(&mut ret,
                    examples,
                    &mut |ex| Path::new("examples").join(&format!("{}.rs", ex.name())));

    test_targets(&mut ret,
                 tests,
                 metadata,
                 &mut |test| {
        if test.name() == "test" {
...
fn build_profiles(profiles: &Option<TomlProfiles>) -> Profiles {
    ...
    fn merge(profile: Profile, toml: Option<&TomlProfile>) -> Profile {
        let &TomlProfile { ref opt_level, lto, codegen_units, debug, debug_assertions, rpath, ref panic } =
            match toml {
                Some(toml) => toml,
                None => return profile,
            };
