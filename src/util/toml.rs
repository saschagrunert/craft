...
pub fn to_manifest(contents: &str,
    ...
    let manifest: TomlManifest = Decodable::decode(&mut d).map_err(|e| human(e.to_string()))?;
...
impl TomlManifest {
    fn to_real_manifest(&self,
        ...
        let project = project.chain_error(|| human("no `package` or `project` section found."))?;
...
impl TomlDependency {
    fn to_dependency(&self, name: &str, cx: &mut Context, kind: Option<Kind>) -> CraftResult<Dependency> {
        ...
        let mut dep = match cx.pkgid {
            Some(id) => DependencyInner::parse(name, version, &new_source_id, Some((id, cx.config)))?,
