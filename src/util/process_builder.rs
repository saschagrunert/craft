...
use util::{CraftResult, ProcessError, process_error, read2};
...
impl ProcessBuilder {
    ...
    pub fn exec_with_streaming(&self,
                               on_stdout_line: &mut FnMut(&str) -> CraftResult<()>,
                               on_stderr_line: &mut FnMut(&str) -> CraftResult<()>)
        ...
        if !output.status.success() {
            ...
            Err(process_error(&format!("failed to parse process output: `{}`", self.debug_string()),
                              Some(Box::new(e)),
                              Some(&output.status),
                              Some(&output)))
        } else {
