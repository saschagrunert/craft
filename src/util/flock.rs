...
use util::{CraftResult, ChainError, Config, human};
...
/// concurrent invocations across multiple instances of Craft.
#[derive(Clone, Debug)]
...
impl Filesystem {
    ...
    pub fn open_rw<P>(&self, path: P, config: &Config, msg: &str) -> CraftResult<FileLock>
        where P: AsRef<Path>
    {
    ...
    /// error.
    pub fn open_ro<P>(&self, path: P, config: &Config, msg: &str) -> CraftResult<FileLock>
        where P: AsRef<Path>
    {
        self.open(path.as_ref(),
    ...
    fn open(&self, path: &Path, opts: &OpenOptions, state: State, config: &Config, msg: &str) -> CraftResult<FileLock> {
...
///
/// Almost all long-running blocking actions in Craft have a status message
...
fn acquire(config: &Config,
    ...
    // there anyway.
    //
    // [1]: https://github.com/rust-lang/craft/issues/2615
    ...
    match try() {
        ...
        Err(e) => {
            if e.raw_os_error() != lock_contended_error().raw_os_error() {
                return Err(human(e)).chain_error(|| human(format!("failed to lock file: {}", path.display())));
            }
        }
    }
