...
//!
//! Most of the time when you're running craft you expect Ctrl-C to actually
...
//! not happen and Ctrl-C just kills craft.
//!
...
mod imp {
    ...
    pub unsafe fn setup() -> Option<()> {
        ...
        // when-craft-is-killed-subprocesses-are-also-killed, but that requires
        // one craft spawned to become its own session leader, so we do that
        // here.
        if env::var("__CRAFT_TEST_SETSID_PLEASE_DONT_USE_ELSEWHERE").is_ok() {
            libc::setsid();
        }
        Some(())
    }
...
mod imp {
    ...
    pub unsafe fn setup() -> Option<Setup> {
        ...
        // to force Craft to abort in this situation or force others to *not*
    ...
    impl Drop for Setup {
        fn drop(&mut self) {
            ...
            unsafe {
                ...
                let mut info: winapi::JOBOBJECT_EXTENDED_LIMIT_INFORMATION;
                info = mem::zeroed();
                let r = kernel32::SetInformationJobObject(self.job.inner,
                                                          winapi::JobObjectExtendedLimitInformation,
                                                          &mut info as *mut _ as winapi::LPVOID,
                                                          mem::size_of_val(&info) as winapi::DWORD);
                if r == 0 {
    ...
    impl Setup {
        unsafe fn kill_remaining(&mut self) -> bool {
            ...
            let r = kernel32::QueryInformationJobObject(self.job.inner,
            ...
            let list = list.iter()
                ...
                .filter_map(|&id| {
                    ...
                    // ignore the problem.
                    let flags = winapi::PROCESS_QUERY_INFORMATION | winapi::PROCESS_TERMINATE | winapi::SYNCHRONIZE;
                    let p = kernel32::OpenProcess(flags, winapi::FALSE, id as winapi::DWORD);
                    if p.is_null() {
                        None
                    } else {
                        Some(Handle { inner: p })
            ...
            for p in list {
                ...
                let mut buf = [0; 1024];
                let r = psapi::GetProcessImageFileNameW(p.inner, buf.as_mut_ptr(), buf.len() as winapi::DWORD);
                if r == 0 {
    ...
    impl Drop for Handle {
        fn drop(&mut self) {
            unsafe {
                kernel32::CloseHandle(self.inner);
            }
        }
    }
}
