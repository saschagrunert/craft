...
impl<'cfg> Workspace<'cfg> {
    ...
    pub fn target_dir(&self) -> Filesystem {
        self.target_dir.clone().unwrap_or_else(|| Filesystem::new(self.root().join("target")))
    }
    ...
    fn find_root(&mut self, manifest_path: &Path) -> CraftResult<Option<PathBuf>> {
        ...
        while let Some(path) = cur {
            let manifest = path.join("Craft.toml");
    ...
    fn find_members(&mut self) -> CraftResult<()> {
        ...
        let members = {
            ...
            match *root.workspace_config() {
                ...
                _ => {
                    bail!("root of a workspace inferred but wasn't a root: {}",
        ...
        if let Some(list) = members {
            ...
            for path in list {
                let manifest_path = root.join(path).join("Craft.toml");
    ...
    fn find_path_deps(&mut self, manifest_path: &Path) -> CraftResult<()> {
    ...
    fn validate(&mut self) -> CraftResult<()> {
        ...
        if !self.members.contains(&self.current_manifest) {
            ...
            let members_msg = match current_dir.strip_prefix(root_dir) {
                Ok(rel) => {
                    format!("this may be fixable by adding `{}` to the `workspace.members` array of the manifest \
                                 located at: {}",
                            rel.display(),
                            root.display())
                }
                Err(_) => {
                    format!("this may be fixable by adding a member to the `workspace.members` array of the \
                                 manifest located at: {}",
                            root.display())
                }
            };
        ...
        if let Some(ref root_manifest) = self.root_manifest {
            ...
            for pkg in self.members().filter(|p| p.manifest_path() != root_manifest) {
                if pkg.manifest().profiles() != &default_profiles {
                    let message = &format!("profiles for the non root package will be ignored, specify profiles at \
                                            the workspace root:\npackage:   {}\nworkspace: {}",
...
impl<'cfg> Packages<'cfg> {
    ...
    fn load(&mut self, manifest_path: &Path) -> CraftResult<&MaybePackage> {
        let key = manifest_path.parent().unwrap();
        match self.packages.entry(key.to_path_buf()) {
            Entry::Occupied(e) => Ok(e.into_mut()),
            Entry::Vacant(v) => {
                let source_id = try!(SourceId::for_path(key));
                let pair = try!(ops::read_manifest(&manifest_path, &source_id, self.config));
                let (manifest, _nested_paths) = pair;
                Ok(v.insert(match manifest {
                    EitherManifest::Real(manifest) => MaybePackage::Package(Package::new(manifest, manifest_path)),
                    EitherManifest::Virtual(v) => MaybePackage::Virtual(v),
                }))
            }
        }
    }
