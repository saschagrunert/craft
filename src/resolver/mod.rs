...
//! ## Performance
//!
//! Note that this is a relatively performance-critical portion of Craft. The
...
impl Resolve {
    pub fn merge_from(&mut self, previous: &Resolve) -> CraftResult<()> {
...
this could be indicative of a few possible situations:
    ...
    * newer Craft implementations know how to checksum this source, but this
...
",
                    ...
                    // that future Craft figured out how to checksum something or
...
",
    ...
    pub fn query(&self, spec: &str) -> CraftResult<&PackageId> {
...
impl DepsFrame {
    ...
    fn min_candidates(&self) -> usize {
        self.remaining_siblings
            .as_slice()
            .get(0)
            .map(|&(_, ref candidates, _)| candidates.len())
            .unwrap_or(0)
    }
}
...
fn activate_deps_loop<'a>(mut cx: Context<'a>,
    ...
    for &(ref summary, ref method) in summaries {
        ...
        remaining_deps.extend(try!(activate(&mut cx, registry, None, candidate, method)));
    }
    ...
    while let Some(mut deps_frame) = remaining_deps.pop() {
        ...
        let my_candidates = {
            ...
            candidates.iter()
                .filter(|&b| {
                    ...
                    prev_active.iter().all(|a| !compatible(a.version(), b.summary.version()))
                })
                .cloned()
                .collect()
        };
        ...
        let candidate = match remaining_candidates.next() {
            ...
            None => {
                ...
                match find_candidate(&mut backtrack_stack,
                                     ...
                                     &mut features) {
                    None => {
                        return Err(activation_error(&cx,
                                                    registry,
                                                    &parent,
                                                    &dep,
                                                    &cx.prev_active(&dep),
        ...
        remaining_deps.extend(try!(activate(&mut cx, registry, Some(&parent), candidate, &method)));
    }
...
fn activation_error(cx: &Context,
                    ...
                    -> Box<CraftError> {
    if candidates.len() > 0 {
        let mut msg = format!("failed to select a version for `{}` (required by `{}`):\nall possible versions \
                               conflict with previously selected versions of `{}`",
                              dep.name(),
                              parent.name(),
                              dep.name());
        'outer: for v in prev_active.iter() {
            for node in cx.resolve_graph.iter() {
                ...
                for edge in edges {
                    if edge != v.package_id() {
                        continue;
                    }

                    msg.push_str(&format!("\n  version {} in use by {}", v.version(), edge));
                    continue 'outer;
                }
            }
            msg.push_str(&format!("\n  version {} in use by ??", v.version()));
        }
    ...
    let msg = format!("no matching package named `{}` found (required by `{}`)\nlocation searched: {}\nversion \
                       required: {}",
                      dep.name(),
                      parent.name(),
                      dep.source_id(),
    ...
    // indicate that we updated a sub-package and forgot to run `craft
    // update`. In this case try to print a helpful error!
    if dep.source_id().is_path() && dep.version_req().to_string().starts_with("=") && !candidates.is_empty() {
        msg.push_str("\nconsider running `craft update` to update \
...
fn build_features(s: &Summary, method: &Method) -> CraftResult<(HashMap<String, Vec<String>>, HashSet<String>)> {
    ...
    match *method {
        Method::Everything => {
            ...
            for dep in s.dependencies().iter().filter(|d| d.is_optional()) {
                try!(add_feature(s, dep.name(), &mut deps, &mut used, &mut visited));
            }
        }
    ...
    match *method {
        ...
        Method::Required { uses_default_features: true, .. } => {
            if s.features().get("default").is_some() {
                try!(add_feature(s, "default", &mut deps, &mut used, &mut visited));
            }
        }
...
impl<'a> Context<'a> {
    ...
    fn flag_activated(&mut self, summary: &Rc<Summary>, method: &Method) -> bool {
        ...
        let (features, use_default) = match *method {
            Method::Required { features, uses_default_features, .. } => (features, uses_default_features),
    ...
    fn build_deps(&mut self,
        ...
        let mut deps = try!(deps.into_iter()
            .map(|(dep, features)| {
                ...
                candidates.sort_by(|a, b| b.summary.version().cmp(a.summary.version()));
                Ok((dep, candidates, features))
            })
            .collect::<CraftResult<Vec<DepInfo>>>());
    ...
    /// return.
    fn query(&self, registry: &mut Registry, dep: &Dependency) -> CraftResult<Vec<Candidate>> {
    ...
    fn resolve_features(&mut self,
                        candidate: &Summary,
                        method: &Method)
                        -> CraftResult<Vec<(Dependency, Vec<String>)>> {
        ...
        let (mut feature_deps, used_features) = try!(build_features(candidate, method));
        let mut ret = Vec::new();
...
fn check_cycles(resolve: &Resolve, activations: &HashMap<(String, SourceId), Vec<Rc<Summary>>>) -> CraftResult<()> {
    ...
    for pkg in all_packages {
        if !checked.contains(pkg) {
            try!(visit(resolve, pkg, &summaries, &mut HashSet::new(), &mut checked))
        }
    }
    return Ok(());
    ...
    fn visit<'a>(resolve: &'a Resolve,
        ...
        if checked.insert(id) {
            ...
            for dep in resolve.deps(id) {
                let is_transitive = summary.dependencies().iter().any(|d| d.matches_id(dep) && d.is_transitive());
